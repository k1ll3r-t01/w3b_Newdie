## Challenge: funjs
### Information
Đây là trang HTML sẽ xuất ra flag nếu bạn nhập đúng dữ liệu vào form, và xuất ra NOP ! nếu bạn nhập sai.  
Hãy phân tích hàm main và tìm ra giá trị nhập vào chính xác!  
### Solution
Mở trang web ta thấy form đang nhảy lambada trên màn hình.
<img src="https://github.com/user-attachments/files/23548577/1.md" width="900">

Khó khăn lắm mới điền và submit được thì vẫn nhận được kết quả là **NOP!**
<img src="https://github.com/user-attachments/files/23548610/2.md" width="900">

Do đó ta sẽ đi mổ xẻ page source.
```html
<html>
    <head>
...
Code khá rối, ta sẽ chú ý dịch một vài phần quan trọng trước:
```js
var flag = document[_0x374fd6(0x183)](_0x374fd6(0x182))['value'],
            _0x4949 = [0x20, 0x5e, 0x7b, 0xd2, 0x59, 0xb1, 0x34, 0x72, 0x1b, 0x69, 0x61, 0x3c, 0x11, 0x35, 0x65, 0x80, 0x9, 0x9d, 0x9, 0x3d, 0x22, 0x7b, 0x1, 0x9d, 0x59, 0xaa, 0x2, 0x6a, 0x53, 0xa7, 0xb, 0xcd, 0x25, 0xdf, 0x1, 0x9c],
            _0x42931 = [0x24, 0x16, 0x1, 0xb1, 0xd, 0x4d, 0x1, 0x13, 0x1c, 0x32, 0x1, 0xc, 0x20, 0x2, 0x1, 0xe1, 0x2d, 0x6c, 0x6, 0x59, 0x11, 0x17, 0x35, 0xfe, 0xa, 0x7a, 0x32, 0xe, 0x13, 0x6f, 0x5, 0xae, 0xc, 0x7a, 0x61, 0xe1],
...
```
Trong đó:
- `document[_0x374fd6(0x183)]['value']` → `document.getElementById('flag').value`
- `flag[_0x374fd6(0x17c)] != 0x24` → `flag.length != 36`
- `text2img(_0x374fd6(0x185))` → `text2img("NOP !")`
  
===> Flag phải dài đúng 36 ký tự. Nếu không, nó hiện "NOP !".  

Sau khi kiểm tra độ dài, code sẽ chạy một vòng lặp 36 lần
```js
for (var i = 0x0; i < flag[_0x374fd6(0x17c)]; i++) {
            if (flag[_0x374fd6(0x176)](i) == operator[i % operator[_0x374fd6(0x17c)]](_0x4949[i], _0x42931[i])) {} else {
                text2img(_0x374fd6(0x185));
...
```
Trong đó:
- `flag[_0x374fd6(0x176)](i)` ➔ `flag.charCodeAt(i)`
- `operator[i % 4](_0x4949[i], _0x42931[i])` ➔ Đây là phép toán tính ra mã ASCII đúng.

===> Lấy ký tự thứ i, so sánh mã ASCII của nó với **kết quả của một phép toán**. Nếu khớp, đi tiếp. Nếu không, báo lỗi.

Ta sẽ viết một đoạn script vào **Console** để xử lí bài toán này
```js
//Sao chép y hệt 3 mảng dữ liệu từ code gốc
...
var operator = [
    (_0x3a6862, _0x4b2b8f) => { return _0x3a6862 + _0x4b2b8f; }, // Phép +
    (_0xa50264, _0x1fa25c) => { return _0xa50264 - _0x1fa25c; }, // Phép -
    (_0x3d7732, _0x48e1e0) => { return _0x3d7732 * _0x48e1e0; }, // Phép *
    (_0x32aa3b, _0x53e3ec) => { return _0x32aa3b ^ _0x53e3ec; }  // Phép XOR
];

//Tạo một chuỗi rỗng để chứa flag
var flag = "";

//Chạy vòng lặp 36 lần để giải mã
for (var i = 0; i < 36; i++) {
    //Tính toán mã ký tự ĐÚNG
    var correct_char_code = operator[i % 4](_0x4949[i], _0x42931[i]);
    
    //Chuyển mã ký tự thành chữ và cộng vào flag
    flag += String.fromCharCode(correct_char_code);
}
...
Cuối cùng **Enter** và ta nhận được flag  
<img src="https://github.com/user-attachments/files/23548844/3.md" width="900">
